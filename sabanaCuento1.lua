...
function scene:createScene( event )
	...
	fondo=display.newImage("img/cu01.png",1024,600)
...
function start( event )
	...
	if event.phase == "began" then
	  ...
	  storyboard.gotoScene("sabanaCuento2","fade",400)
	end
end
...
function texto0( event )
	...
	contador=2
	text1.text="Ernestino es un cocodrilo que vive en la Sabana,".. "\nsu casa es un refrescante lago, el cual comparte" .. "\ncon su amigo Agustín, el hipopótamo"
	transition.fadeIn( text1, {time=1000} )

	channel=audio.findFreeChannel()
	sonido=audio.loadStream("music/cuento1/Parrafo 1.mp3", {loops = -1, channel = channel})
	audio.play(sonido)
...
function texto1( event )
	...
	contador=3
	sonido=audio.loadStream("music/cuento1/Parrafo 1_2.mp3", {loops = -1, channel = channel})
	audio.play(sonido)
...
function scene:exitScene( event )
   ...
   dicci:removeEventListener( "touch", fade_out)
   cancelAll()
   storyboard.removeScene("sabanaCuento1")
