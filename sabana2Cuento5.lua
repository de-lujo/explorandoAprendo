...
function scene:createScene( event )
	...
	fondo=display.newImage("img/cu205.png",1024,600)
	...
	text1= display.newText("Todos los animales decidieron reunirse en un gran".. "\nencuentro y resolver la situación, pensaron y pensaron".."\ny no encontraban solución.", 0, 0, native.systemFont, 18 )
...
function texto1( event )
	text1.text= "Las hienas que eran muy curiosas y observadoras," .. "\nse observaron fijamente de pies a cabeza y…" .. "\n\ncomenzaron a reír tan fuerte de lo gracioso" .. "\nque ambos se veían:"
	transition.fadeIn( text1, {time=1000} )
end

function texto2( event )

	text1.text= "luego de reír tanto tantísimo decidieron aclarar" .. "\ntamaña confusión: ¡Ahí está la culpable!".. "\n\nEra la juguetona y traviesa amiga Suricata." .. "\n(Al final todos se reían: ¡JIJIJI!)"
	transition.fadeIn( text1, {time=1000} )
...
function back( event )
	...
	if event.phase == "began" then
	  ...
	  storyboard.gotoScene("sabana2Cuento4","fade",400)
	end
end
...
function scene:enterScene( event)
	...
	sonido=audio.loadStream("music/cuento2/Parrafo 10.mp3", {loops = -1, channel = channel})
	audio.play(sonido)
	...
	time[2]=timer.performWithDelay(20000, destexto,1)
	time[3]=timer.performWithDelay(21000, texto2, 1)
...
function scene:exitScene( event )
   ...
   corchea:addEventListener( "touch", sin_musica )
   dicci:addEventListener( "touch", fade_out)
   storyboard.removeScene("sabana2Cuento5")
